...
class Processor:
    def __init__(
        self, youtube_language: SupportedLanguages, llm: LLMConfig, prompt_factory: PromptFactory
    ):
    ...
    def should_process(self) -> bool:
        return self.youtube_language != SupportedLanguages.ENGLISH

    @execution_time
    async def translate(self, original_script: str) -> str:
        print(TRANSLATING_MESSAGE.format(self.youtube_language.value.capitalize()))
        prompt = self.prompt_factory.prompt(
            ...
            placeholders={
                ...
                PARSED_FORMAT_KEY: self.parser_format(TranslatedSegment),
            },
        )

        response: TranslatedSegment = await self.provider.generate(
            prompt=prompt.template, parser=TranslatedSegment
        )
    ...
    async def validate(self, original_script: str, translated_script: str) -> TranslationValidation:
        ...
        print(VALIDATING_MESSAGE.format(self.youtube_language.value.capitalize()))
        prompt = self.prompt_factory.prompt(
            ...
            placeholders={
                ...
                PARSED_FORMAT_KEY: self.parser_format(TranslationValidation),
            },
        )

        response: TranslationValidation = await self.provider.generate(
            prompt=prompt.template, parser=TranslationValidation
        )
    ...
    async def process(self, original_script: str) -> ProcessedScript:
        ...
        validation = await self.validate(
            original_script=original_script, translated_script=translated_script
        )
